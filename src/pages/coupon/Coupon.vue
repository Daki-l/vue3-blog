<template>
    <div class="coupon_wrapper" v-loading.fullscreen.lock="fullscreenLoading">
        ...
        <div class="input_list">
            <div>
                <label for="hiveid">Hive ID (多个用分号分隔):</label><br>
                <el-input type="text" id="hiveid" v-model="hiveIds"></el-input>
            </div>
            <div>
                <label for="coupon">兑换码 (多个用分号分隔):</label><br>
...
import { getSummonerCode, getSummonerUserInfo, setCodeToUser } from "@/services/summonerServices/summonerServices.js";
...
let getCodeList = async () => {
    ...
    getSummonerCode().then(res => {
        console.log(' codeList.value-x2-',  res);
...
let setCodeList = () => {
    let codesStr = codeList.value.map(e => {
        return e.coupon;
    }).join(';')
...
let getUserInfo = () => {
    ...
    hiveidsList.forEach(e => {
        let params = {
            country: 'CN', lang: 'zh-hans', server: 'china', hiveid: e, coupon: 'xxx'
        };
        promiseList.push({
    ...
    Promise.all(promiseList.map(e => e.fn)).then(data => {
        let list = [];
        data.forEach((e, endex) => {
            let curUser = promiseList[endex];
            let { retCode, retMsg, userData = {} } = e;
            if (retCode != 100) {
                list.push({ result: { retCode, retMsg }, hiveid: curUser.hiveid, coupon: curUser.coupon, });
            } else {
...
let sendRequests = async () => {
    ...
    for (let hiveid of hiveidsList) {
        for (let coupon of couponsList) {
            let params = {
                country: 'CN',lang: 'zh-hans',server: 'china',hiveid,coupon
            };
            promiseList.push({
...
let getCouponContent = (coupon) => {
    let curCoupon = codeList.value.find(e => {
